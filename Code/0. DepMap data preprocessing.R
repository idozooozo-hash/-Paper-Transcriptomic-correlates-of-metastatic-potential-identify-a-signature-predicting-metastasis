...
tpm=tpm[,csams]
tpm=t(tpm)
tpm=cbind(data.frame(Sample=rownames(tpm)),tpm)
...
# save(rcm, file=paste0(dir_data,"0. read_count(DepMap).Rdata"))
# save(sinfo, file=paste0(dir_data,"0. all_sample_information(DepMap).Rdata"))

sinfo=sinfo[match(csams,sinfo$Sample),]
# save(sinfo, file=paste0(dir_data,"0. filtered_sample_information(DepMap).Rdata"))